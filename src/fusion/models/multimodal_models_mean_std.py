...
from fusion.models.common_layers import MultimodalDownsamplerS32MeanSTD, PredictionsUpsampler, TripleFusion, DoubleFusion
...
class LabelEncoderFusionDFAV(torch.nn.Module):
    def __init__(self, out_emo: int = 6, out_sen: int = 3):
        ...
        self.downsampler = MultimodalDownsamplerS32MeanSTD(features_only=False, out_f_size=1024)
        self.ap_upsampler = PredictionsUpsampler(inp_size=(out_emo + out_sen) * 2, out_t_size=20, out_f_size=1024)
        self.vp_upsampler = PredictionsUpsampler(inp_size=(out_emo + out_sen) * 2, out_t_size=20, out_f_size=1024)
        self.tp_upsampler = PredictionsUpsampler(inp_size=(out_emo + out_sen), out_t_size=20, out_f_size=1024)
...
if __name__ == "__main__":
    ...
    feats = []
    for feats_size in [(batch_size, 199, 1024 * 2), (batch_size, 20, 512 * 2), (batch_size, 32, 1024), 
                       (batch_size, (out_emo + out_sen) * 2), 
                       (batch_size, (out_emo + out_sen) * 2), 
